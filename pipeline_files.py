...
def calculate_divisiveness(
    ...
    if method in ["std", "std_rank"]:
        ...
        for i, tmp in data.groupby(location_level):
            for cnd, tmp_cand in tmp.groupby(candidate):
                output.append({
                    "candidate": cnd,
        ...
        for idx, dt in data.groupby(location_level):
            ...
            v, df_between = competitiveness(dt, votes="value", unit="polling_id", candidate=candidate)
            df_between["type"] = "EC"

            v, df_within = polarization(dt, votes="value", unit="polling_id", candidate=candidate)
            df_within["type"] = "EP"
    ...
    if flag_candidates == 0:
        rnd = f"{rnd}_all"
    # path = f"data_curated/{country}/antagonism_{year}_{location_level}_{rnd}.csv.gz"

    path = f"data_curated/{country}/antagonism_{year}_{location_level}_{rnd}_{mname}.csv.gz"

...
```
```{r}
coefs.1 <- unname(unlist(summary(model.1)$coefficients[,1]))
pvals.1 <- unname(unlist(summary(model.1)$coefficients[,4]))
dt.1 <- data.frame(A=coefs.1, B=pvals.1, C=c(7,6,5,4,3,2,1))

coefs.2 <- unname(unlist(summary(model.2)$coefficients[,1]))
pvals.2 <- unname(unlist(summary(model.2)$coefficients[,4]))
...
plot_summs(model.1, model.2, plot.distributions = FALSE, 
           model.names = c("Mass Polarization", "Political Interest"),
           legend.title="Dependent Variable",
           coefs=c("EC"="scale(ec)", "EP"="scale(ep)", "Gini"="scale(gini)", "Unemployment"="scale(unemployment)", "GDP PC"="scale(gdp)", "Pop Density"="scale(pop_density)", "Ed. Attainment"="scale(ed_attainment)"),
...
ggsave("C:/Users/cnava/OneDrive/Documentos/Figures_Papers/ElectionPolarization/Figure4.pdf")
```

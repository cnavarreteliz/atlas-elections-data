...
nls_extract_params <- function(model) {
    ...
    nobs <- summary(model)[]$N[[1]]
    deviance <- summary(model)[]$deviance[[1]]
    adjrsquared <- summary(model)[]$adj.r.squared[[1]]
    rsquared <- summary(model)[]$r.squared[[1]]
    rse <- summary(model)[]$rse[[1]] # Residual Std. Error rse
    rdf <- summary(model)[]$rdf[[1]] # Residual Std. Error rse
...
customstargazer <- function(models, digits = 4, sort_parameters = c(), rename_parameters = c()) {
    ...
    fstat_row <- c("F-stat")
    rsquared_row <- c("R2")
    adj_rsquared_row <- c("Adjusted R2")
    res_std_error_row <- c("Residual Std. Error")
    df_row <- c("df")
    ...
    for (model in models) {
        ...
        adj_rsquared_row <- append(adj_rsquared_row, list(round(ret[[5]], digits = 3)))
        rsquared_row <- append(rsquared_row, list(round(ret[[6]], digits = 3)))
        res_std_error_row <- append(res_std_error_row, list(round(ret[[7]], digits = 3)))
        df_row <- append(df_row, list(round(ret[[8]], digits = 2)))
    }
    ...
    for (model in it_models) {
        ...
        pvalue_column <- "Pr(>|t|)" # Pr(> |z|) Pr(>|t|)
        std_column <- "Std. Error" # Std. Error
        value_column <- "t value" # "t value"
    ...
    df_output[nrow(df_output) + 1, ] <- rsquared_row
    df_output[nrow(df_output) + 1, ] <- res_std_error_row
    df_output[nrow(df_output) + 1, ] <- df_row
